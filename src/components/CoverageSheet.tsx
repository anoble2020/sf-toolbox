  ...
  const fetchCoverage = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/salesforce/coverage?instance_url=${encodeURIComponent(instance_url)}`,
        {
          headers: {
                ...
                <TableBody>
                  {uniqueCoverage.map((item) => {
                    ...
                    const coveragePercent = totalLines > 0 
                      ? Math.round((item.NumLinesCovered / totalLines) * 100)
                      : 0
                        ...
                        <TableCell>
                          <div className="flex items-center gap-4">
                            <Progress 
                              ...
                              className={cn(
                                "w-[100px]",
                                coveragePercent < 75 ? "text-red-500" : "text-green-500"
                              )}
                            />
