    ...
    <div className="relative">
      <div className="flex items-center h-12 group hover:bg-gray-50">
        ...
        <div 
          ...
          style={{ width: `${labelWidth}px`, paddingLeft: `${event.level * 20}px` }}
        >
        ...
        <div 
          ...
          onClick={() => onEventClick?.(event.lineNumber)}
        >
          <div 
...
const TimeAxis = ({ scale, height, ticks, formatTime, labelWidth }: {
  ...
  return (
    <div className="relative" style={{ height: `${height}px`, marginLeft: `${labelWidth}px` }}>
      {tickValues.map((tick, i) => (
        ...
        >
          <div className="relative -top-4 -translate-x-1/2 text-xs text-gray-500">
            {formatTime(tick)}
          </div>
        </div>
      ))}
...
export function TraceViewer({ content, onClose, onEventClick }: TraceViewerProps) {
  ...
  const maxWidth = typeof window !== 'undefined' ? 
    window.innerWidth * 0.9 - LABEL_WIDTH - RIGHT_MARGIN : 1000;
...
function parseLogEvents(content: string): TraceEvent[] {
  ...
  lines.forEach((line, lineNumber) => {
    const match = line.match(/(\d{2}:\d{2}:\d{2}\.\d+)\s*\((\d+)\)\|CODE_UNIT_(STARTED|FINISHED)\|.*?\|([^|]+)(?:\|([^|]+))?/);
    if (!match) return;
